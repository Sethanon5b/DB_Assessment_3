	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexOut i) : SV_Target
			{
				...
				float calc000 = pi + atan2(rotateduv01.y, rotateduv01.x) - _Arc * pi2;
				float lineMul = calc000 * (pi2 / (pi2 - _Arc * pi2));
				...
				//////////////////////////////////////////////////INNER_SPACING
				float innerspacing = clamp(finalArc + mul(1 - clamp((lines - _SegmentSpacing) / fwidth(lines - _SegmentSpacing), 0, 1),
										 mul(clamp(remap(_SegmentSpacing, float2(0, 0.001), float2(0, 1)), 0, 1),
											 round(clamp(remap(_SegmentCount, float2(1, 2), float2(0, 0.51)), 0, 1)))), 0, 1);
				...
				{
					selectedInnerGradientCoord = float2(1/(1 - _Arc), 1) * rotateuv(
						polarCoordinates(
							rotateuv(transuv, float2(0, 0),radians(_Rotation - 90)),
							float2(0, 0),1,1
						),
						float2(0, 0),pi/2
					) + float2(.5, 0);
				...
				float borderShape = clamp(clamp((borderCircle - borderSpacing_border) - (removedsegments * _RemoveBorder), 0, 1) - clamp(segcirc, 0, 1), 0, 1);
